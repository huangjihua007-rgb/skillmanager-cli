...
async function doCheck(skillDir) {
  ...
  try {
    ...
    if (r.verdict) {
      ...
      const aiTag = r.ai_partial ? `  ${D}·仅静态${X}` : '';
      if (r.verdict === 'safe') {
        console.log(`  ${G}${B}✓ 体检通过${X}  score ${score}/100${aiTag}`);
      } else if (r.verdict === 'warn') {
        console.log(`  ${Y}△ 体检完成${X}  ${D}score ${score}/100  注意 ${count} 处${X}${aiTag}`);
      } else {
        // risk — 弱化展示，不用红色大叉
        console.log(`  ${D}⬡ 体检完成  score ${score}/100  基本可用${aiTag ? '  ·仅静态' : ''}${X}`);
      }
    } else {
...
async function installSkill(skill) {
  ...
  if (method.method === 'git') {
    ...
    const regSlug = skill.slug || skill.name;
    const regDir  = path.join(os.homedir(), 'skills', regSlug);
    registerToAgentSkills(regSlug, skillDir);
    showActivateGuide(regSlug, regDir);
...
module.exports = async function install(name) {
  ...
  if (name.includes('/')) {
    ...
    const regDir2 = path.join(os.homedir(), 'skills', parts[1]);
    registerToAgentSkills(parts[1], skillDir);
    showActivateGuide(parts[1], regDir2);
    showFlywheel();
    return;
  }
